...
def comp_multadds(model, input_size=(512,512), half=False):
    #input_size = (1,) + tuple(input_size)
    model = model.cuda()
    input_data1 = torch.randn((1,3,512,512)).cuda()
    input_data2 = torch.randn((1,1,512,512)).cuda()
    model = add_flops_counting_methods(model)
    
    model.start_flops_count()
    with torch.no_grad():
        _ = model(input_data1, input_data2, input_data1, input_data2)
