...
from .SaGate import SAGate

def default_conv(in_channels, out_channels, kernel_size, stride=1, dilation=1, bias=True):
    padding = kernel_size // 2 if dilation==1 else 2
    return nn.Conv2d(in_channels, out_channels, kernel_size,  stride=1, padding=padding, dilation=dilation, bias=bias)
                         
class BBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, dilation=1, batchNorm_type=0, act=None):
        super(BBlock, self).__init__()

        m = []
        m.append(default_conv(in_channels, out_channels, kernel_size=kernel_size, stride=stride, dilation=dilation))
        if(batchNorm_type == 1):
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, outplanes, kernel_size=3, stride=1, dilation=1, batchNorm_type=1, inplace=True):
        super(BasicBlock, self).__init__()
        self.inplanes  = inplanes
        self.outplanes = outplanes
        
        self.block1 = BBlock(inplanes, outplanes, kernel_size=kernel_size, stride=1, dilation=1, batchNorm_type=batchNorm_type, act=nn.ReLU(inplace))
        self.block2 = BBlock(outplanes, outplanes, kernel_size=kernel_size, stride=1, dilation=1, batchNorm_type=batchNorm_type, act=nn.ReLU(inplace))
...
class lightingNet(nn.Module):
    ...
    def forward(self, innerFeat, guide_feat, lights=None, tmps=None):
        ...
        return innerFeat, glight, gtmp

class DGateNet(nn.Module):
    def __init__(self, ):
        super(DGateNet, self).__init__()
        self.inChannel   = 3
        self.baseFilter  = 48
        ...
        self.pre_block = BasicBlock(self.inChannel, self.baseFilter, kernel_size=5, batchNorm_type=1)  # 3->48
        self.depth_pre_block = BasicBlock(1, self.baseFilter, kernel_size=5, batchNorm_type=1)         # 1->48
        ...
        self.HG0_gate       = SAGate(self.ncHG0, self.ncHG0)
        ...
        self.HG1_gate       = SAGate(self.ncHG1, self.ncHG1)
        ...
        self.HG2_gate       = SAGate(self.ncHG2, self.ncHG2)
        ...
        self.HG3_gate       = SAGate(self.ncHG3, self.ncHG3)
    ...
    def forward(self, input_img, input_depth, guide_img, guide_depth, lights=None, tmps=None): 
        ### guide image
        ...
        feat_guide_gate, depth_guide_gate, guide_merge_gate = self.HG0_gate(feat_guide, depth_guide)
        
        feat_guide  = self.HG1_downSample(feat_guide + feat_guide_gate)
        depth_guide = self.HG1_downSample(depth_guide + depth_guide_gate)
        feat_guide  = self.HG1_low1(feat_guide)
        depth_guide = self.HG1_low1_depth(depth_guide)
        feat_guide_gate, depth_guide_gate, guide_merge_gate = self.HG1_gate(feat_guide, depth_guide)
        
        feat_guide  = self.HG2_downSample(feat_guide + feat_guide_gate)
        depth_guide = self.HG2_downSample(depth_guide + depth_guide_gate)
        feat_guide  = self.HG2_low1(feat_guide)
        depth_guide = self.HG2_low1_depth(depth_guide)
        feat_guide_gate, depth_guide_gate, guide_merge_gate = self.HG2_gate(feat_guide, depth_guide)
        
        feat_guide  = self.HG3_downSample(feat_guide + feat_guide_gate)
        depth_guide = self.HG3_downSample(depth_guide + depth_guide_gate)
        feat_guide  = self.HG3_low1(feat_guide)
        depth_guide = self.HG3_low1_depth(depth_guide)
        feat_guide_gate, depth_guide_gate, guide_merge_gate = self.HG3_gate(feat_guide, depth_guide)
        ...
        feat_input_gate, depth_input_gate, input_merge_gate = self.HG0_gate(feat_input, depth_input)
        
        feat1_upper = self.HG1_upper(input_merge_gate)
        feat_input  = self.HG1_downSample(feat_input+feat_input_gate)
        depth_input = self.HG1_downSample(depth_input+depth_input_gate)
        feat_input  = self.HG1_low1(feat_input)
        depth_input = self.HG1_low1_depth(depth_input)
        feat_input_gate, depth_input_gate, input_merge_gate = self.HG1_gate(feat_input, depth_input)

        feat2_upper = self.HG2_upper(input_merge_gate)
        feat_input  = self.HG2_downSample(feat_input+feat_input_gate)
        depth_input = self.HG2_downSample(depth_input+depth_input_gate)
        feat_input  = self.HG2_low1(feat_input)
        depth_input = self.HG2_low1_depth(depth_input)
        feat_input_gate, depth_input_gate, input_merge_gate = self.HG2_gate(feat_input, depth_input)
        
        feat3_upper = self.HG3_upper(input_merge_gate)
        feat_input  = self.HG3_downSample(feat_input+feat_input_gate)
        depth_input = self.HG3_downSample(depth_input+depth_input_gate)
        feat_input  = self.HG3_low1(feat_input)
        depth_input = self.HG3_low1_depth(depth_input)
        feat_input_gate, depth_input_gate, input_merge_gate = self.HG3_gate(feat_input, depth_input)

        light_feat, guide_dir, guide_tmp = self.lightnet(input_merge_gate, guide_merge_gate, lights, tmps)
        ...
        return out_img, guide_dir, guide_tmp
